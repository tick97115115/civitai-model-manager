File download process not only need communicate with CivitAI and Gopeed service, but also need to:
1. locate the correct file download url
2. file type
3. file name 
4. where should it be downloaded to

First let's take a look at the modelId data that we will take care of in the process.
...
Our goals:
1. Download specific model version file.
	1. where is the destination folder?
	2. how is the folder structured?
	3. how should the downloaded model version file be named?
2. Download images for a specific model version file.
	1. where is the destination folder?
	2. how is the files structured?
	3. how should the downloaded model version file be named?

# Main Content
## Download specific model version file

If model is a checkpoint, then under a checkpoint folder:
- /{checkpoints}
	- /civitai
		- /{modelId}
			- /{modelVersionId}
				- /{versionId} + filename.safetensors
				- /{versionId} + filename.preview.xxx(png,jpg...)

else if model is a lora(or Dora, Lycoris...etc) under a loras folder.(etc)
